...
REGISTRATION_TIMEOUT_DAYS = getattr(settings, 'STEPPING_OUT_REGISTRATION_TIMEOUT_DAYS', 1)
EMAIL_TIMEOUT_DAYS = getattr(settings, 'STEPPING_OUT_EMAIL_TIMEOUT_DAYS', 1)
...
class EmailTokenGenerator(PasswordResetTokenGenerator):
	"""
	Strategy object used to generate and check tokens for a useremail adding mechanism.
	"""
	def make_token(self, user, email):
		"""
		Returns a token that can be used once to attach a useremail to a user.
		"""
	...
	def _make_token_with_timestamp(self, user, email, timestamp):
		...
		hash = sha_constructor(
			settings.SECRET_KEY +\
			unicode(user.id) +\
			unicode(getattr(email.user, 'id', None)) +\
			email.email +\
			unicode(timestamp)
		).hexdigest()[::2]
		return '%s-%s' % (ts_b36, hash)
